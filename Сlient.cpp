...
void saveMessageToFile(const std::string& login, const std::string& message) {
    std::string filename = login + "_messages.txt";
    ...
    if (file.is_open()) {
        ...
        setColor("red");
        printf("Error: Unable to save message to file.\n");
        setColor("white");
    }
}

std::vector<std::string> loadMessagesFromFile(const std::string& login) {
    std::string filename = login + "_messages.txt";
...
void showHistory(const std::string& login) {
    std::vector<std::string> messages = loadMessagesFromFile(login);
    setColor("white");
    printf("History\n");
...
bool processInstructions(std::vector<std::string> splittedMessage) {
    ...
    if (splittedMessage[0] == "!login" && splittedMessage.size() == 3) {
        ...
        // Load message history after login
        setColor("grey");
        showHistory(login);
    ...
    if (splittedMessage[0] == "!nickname" && splittedMessage.size() == 2) {
        setColor("blue");
        printf("Your nickname was changed from \'%s\' -> \'%s\'\n\n", nickname.c_str(), splittedMessage[1].c_str());
        nickname = splittedMessage[1];
        saveNicknameToFile(nickname);
        setColor("white");
        return true;
